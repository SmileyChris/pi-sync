...
If more than `MASS_DELETE_LIMIT` (5) files vanish in a single watcher flush, the entire batch is **aborted**. The user must either restore the files on disk or use `/sync:trash empty <path>` per file to confirm.
